...
public class HashtableClosedAddressImpl<T> extends AbstractHashtableClosedAddress<T> {
	...
	public void insert(T element) {
		if (element != null) {
			...
			if (table[index] == null) {
				List<T> linked = new LinkedList<T>();
				linked.add(element);
				table[index] = linked;
			}

			else {
				((List<T>) table[index]).add(element);
				COLLISIONS++;
			}
			elements++;
		}
	...
	public void remove(T element) {
		if (element != null && search(element) != null) {
			int index = ((HashFunctionClosedAddress<T>) hashFunction).hash(element);
			if (((List<T>) table[index]).size() == 1)
				table[index] = null;

			else {
				((List<T>) table[index]).remove(element);
				COLLISIONS--;
			}
	...
	public T search(T element) {
		...
		if (element != null) {
			int index = ((HashFunctionClosedAddress<T>) hashFunction).hash(element);
			if (table[index] != null) {
				if (((List<T>) table[index]).contains(element))
					retorno = element;
			}
		}
		return retorno;
